...
class Signup
{
	...
	public function evaluate($data)
	{
		...
		foreach ($data as $key => $value) {
			...
			if($key == "email")
			{
				if (!preg_match("/([\w\-]+\@[\w\-]+\.[\w\-]+)/",$value)) {
 					...
 					$this->error = $this->error . "correo invalido!<br>";
    			}
			}
			...
			if($key == "first_name")
			{
				if (is_numeric($value)) {
 					...
 					$this->error = $this->error . "El nombre no puede contener numero<br>";
    			}

    			if (strstr($value, " ")) {
        
 					$this->error = $this->error . "El nombre no puede contener espacios<br>";
    			}
			...
			if($key == "last_name")
			{
				if (is_numeric($value)) {
 					...
 					$this->error = $this->error . "El apellido no puede contener<br>";
    			}

    			if (strstr($value, " ")) {
        
 					$this->error = $this->error . "El apellido no puede contener espacios<br>";
    			}
		...
		if(is_array($check)){
			 ...
			 $this->error = $this->error . "El correo ingresado ya es usado por otro usuario<br>";
		}
